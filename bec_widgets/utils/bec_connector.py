...
class Worker(QRunnable):
    ...
    def run(self):
        ...
        try:
            ...
            self._emit_outcome(self.signals.failed, error_msg)
        else:
            self._emit_outcome(self.signals.completed)

    @staticmethod
    def _emit_outcome(signal, *args):
        """Emit the outcome signal, tolerating a signal source deleted mid-task.

        During application shutdown the Qt side of ``WorkerSignals`` can be destroyed
        while the task is still running; the late emit must not escape ``run`` as an
        unhandled exception in the thread pool.
        """
        try:
            signal.emit(*args)
        except RuntimeError:
            logger.debug("Worker outcome signal emitted after its source was deleted")
